...
RSpec.describe 'PaymentsRequests' do
  include_context 'headers'
  before { Stripe.api_key ||= ENV.fetch('STRIPE_API_KEY', nil) }

  let(:payments) { [user_payment, admin_payment] }
  let(:book) { create(:book, price_cents: 299) }
  let(:admin_payment) { create(:payment, book:, user: admin) }
  let(:user_payment) { create(:payment, book:, user:) }
    ...
    context 'with authentcation' do
      describe 'default behavior' do
        context 'when regular user' do
          ...
          it 'returns users payments' do
            expect(response).to have_http_status :ok
            expect(response_data.count).to eq(Payment.where(user:).count)
        ...
        context 'when admin' do
          ...
          it 'returns all payments' do
            expect(response).to have_http_status :ok
            expect(response_data.count).to eq Payment.count
            expect(response_data).to all(include(*PaymentPresenter.build_attributes))
          end
        end
      end
      ...
      describe 'fields picking' do
        ...
        before do
          ...
          get "#{payments_path}#{query}", headers:
        end
        ...
        context 'without the fields parameter' do
          ...
          it 'returns all fields specified in resource presenter' do
            expect(response).to have_http_status :ok
            expect(response_data.map(&:keys)).to all(eq(PaymentPresenter.build_attributes))
          end
        end
      ...
      describe 'sorting' do
        ...
        context 'with valid sorting params' do
          let(:sorted) do
            response_data.map { |payment| payment['id'] }
          end
          let(:expected) { Payment.order(query_params.values.join(' ')).map(&:id) }
  ...
  describe 'GET /api/payments/:id' do
    context 'without authentication' do
      before do
        authenticate_client
        get(payment_path(user_payment), headers:)
      end
    ...
    context 'with authentcation' do
      before do
        authenticate_client
        authenticate_user(user:)
        get payment_path(id), headers:
      end

      context 'when payment exists' do
        let(:id) { user_payment.id }
  ...
  describe 'POST /api/payments' do
    let(:params) { { data: payment_attributes } }
    let(:payment_attributes) { attributes_for(:payment, book_id: book.id, token:) }
    ...
    context 'without authentication' do
      before do
        VCR.use_cassette('/api/payments/valid_params') do
      ...
      context 'with valid parameters' do
        it 'adds a record to db and returns created resource with location' do
          ...
          expect(response.headers['Location']).to eq(
            "http://www.example.com/api/payments/#{created.id}"
          )
        end
      end

      context 'with invalid parameters' do
        let(:payment_attributes) { { book_id: book.id, token: '' } }

        it 'does not add a record to db, returns HTTP status 422 with error details' do
          post(payments_path, params:, headers:)
          expect(response).to have_http_status :unprocessable_entity
          expect(Payment.find_by(book_id: book.id)).to be_nil
